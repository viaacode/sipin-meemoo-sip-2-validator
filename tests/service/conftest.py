...
def pulsar_container(pulsar_config: dict[str, Any]) -> DockerContainer:
    return DockerContainer("apachepulsar/pulsar").with_command("bin/pulsar standalone").with_bind_ports(6650, pulsar_config["port"])

@pytest.fixture(scope="module", autouse=True)
def pulsar_setup_and_teardown(request: pytest.FixtureRequest, pulsar_container: DockerContainer, pulsar_config: dict[str, Any]):
